...
@Component({
  selector: 'app-venta-municipio',
  templateUrl: './venta-municipio.component.html',
  styleUrls: ['./venta-municipio.component.css']
})
export class VentaMunicipioComponent implements OnInit {
...
generateReportVentaZona()  {
    ...
    this.repS.getClienteZona(this.fechaI, this.fechaF).subscribe((data: any[]) => {
      this.listVen = data;
if (this.listVen.length === 0) {
              swal.fire('No existen datos!!');
}else{
const tableHeaders = ['Municipios', 'Total Clientes','Total Ventas']; 

  const tableRows = this.listVen.map(item => [item.nombremunicipio,item.ncliente, '$'+item.tventa]);

  const table = {
  table: {
    headerRows: 1,
    widths: [255,100,130], // Ajusta los anchos de las columnas según tus necesidades
    body: [
...
//
  ...
  const reporte= 'VENTAS POR MUNICIPIOS.';
...
const fechaF2 = this.datePipe.transform(this.fechaF, 'dd/MM/yyyy');
  ...
  const docDefinition = {
    content: [
      headerContent,
            { text: 'Total de Ventas por Municipios del '+fechaI2+' al '+fechaF2, fontSize: 16, bold: true, margin: [0, 0, 0, 10], alignment: 'center' },
      table,
    ],
     defaultStyle: {
    fontSize: 11,
