...
@Component({
  selector: 'app-egresos-fecha',
  templateUrl: './egresos-fecha.component.html',
  styleUrls: ['./egresos-fecha.component.css']
})
export class EgresosFechaComponent implements OnInit {
  ...
  getAll(){
     this.repS.geCompraFecha().subscribe((data: any[]) => {
      this.ingresos = data;
...
//año
  ...
  const reporte= 'TOTAL DE EGRESOS DIARIOS, MENSUAL Y ANUAL.';
...
const formattedTime = currentDate.toLocaleTimeString();
  ...
  const docDefinition = {
    content: [
      headerContent,
            { text: 'Total de Egresos Diarios, Mensual y Anual' , fontSize: 16, bold: true, margin: [0, 0, 0, 10], alignment: 'center' },
