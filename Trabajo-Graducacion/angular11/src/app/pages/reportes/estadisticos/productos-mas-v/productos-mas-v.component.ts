...
@Component({
  selector: 'app-productos-mas-v',
  templateUrl: './productos-mas-v.component.html',
  styleUrls: ['./productos-mas-v.component.css']
})
export class ProductosMasVComponent implements OnInit {
...
listPF:any[]=[];
 productosRango:any[]=[];
  ...
  generateReportEgresos(){
            this.repS.getProductoMasF(this.fechaI, this.fechaF).subscribe((data: any[]) => {
      this.listPF = data;
...
//
  ...
  const reporte= 'PRODUCTOS TERMINADOS MÁS VENDIDOS.';
...
const formattedTime = currentDate.toLocaleTimeString();
  ...
  const docDefinition = {
    ...
    content: [
         headerContent,
            { text: 'Productos más Vendidos del '+this.fechaI+' al '+this.fechaF ,fontSize: 16, bold: true, margin: [0, 0, 0, 10], alignment: 'center' },
