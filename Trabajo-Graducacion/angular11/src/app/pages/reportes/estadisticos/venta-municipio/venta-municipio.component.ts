...
const tableHeaders = ['Municipio', 'Total Venta']; 

  const tableRows = this.listVen.map(item => [item.nombremunicipio, '$'+item.tventa]);

  const table = {
  table: {
    headerRows: 1,
    widths: [320,180], // Ajusta los anchos de las columnas según tus necesidades
    body: [
...
const formattedTime = currentDate.toLocaleTimeString();
  ...
  const docDefinition = {
    content: [
      headerContent,
            { text: 'Total de Ventas por Municipios del '+this.fechaI+' al '+this.fechaF, fontSize: 16, bold: true, margin: [0, 0, 0, 10], alignment: 'center' },
      table,
    ],
     defaultStyle: {
    fontSize: 11,
