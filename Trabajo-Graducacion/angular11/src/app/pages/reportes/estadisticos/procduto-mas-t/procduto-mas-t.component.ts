...
@Component({
  selector: 'app-procduto-mas-t',
  templateUrl: './procduto-mas-t.component.html',
  styleUrls: ['./procduto-mas-t.component.css']
})
export class ProcdutoMasTComponent implements OnInit {
...
"fechaFin":this.fechaF
            ...
            this.repS.getProductoMasTal(arre).subscribe((data: any[]) => {
      this.listPF = data;
if (this.listPF.length === 0) {
              swal.fire('No existen Datos!!');

}else{
const tableHeaders = ['Códigos','Estilos','# de Talla', '# de Ventas'];
...
this.productosRango=this.listPF;
  ...
  const tableRows = this.productosRango.map(item => [item.codigo.toString().padStart(4, '0'), item.estilo, item.nombre_talla, item.total_vendido]);
...
//
  ...
  const reporte= 'TALLAS DE ZAPATOS MÁS DEMANDADAS POR ESTILOS.';
...
const formattedTime = currentDate.toLocaleTimeString();
  ...
  const docDefinition = {
    ...
    content: [
         headerContent,
            { text: 'Tallas de Zapatos más Demandadas del '+this.fechaI+' al '+this.fechaF ,fontSize: 16, bold: true, margin: [0, 0, 0, 10], alignment: 'center' },
