...
import { ColormpService } from './services/colormp.service';
...
@Component({
  selector: 'app-color-materia',
  templateUrl: './color-materia.component.html',
  styleUrls: ['./color-materia.component.css']
})
export class ColorMateriaComponent implements OnInit, OnDestroy
{
...
id_bitacora:number;
  ...
  constructor(public  regresarService: RegresarService, private service: ColormpService, 
     private router: Router,
     private route: ActivatedRoute, private bitacoraS:BitacoraService, private loginS: LoginService
) { 
this.update2=false;
this.col= new Colormp();
}
...
this.update2=true;
  ...
  this.id_color = id_color;
    ...
    this.service.find(id_color).subscribe((data: Colormp)=>{
      this.col = data;
...
getAll():void{
  this.service.getAll().subscribe((data: Colormp[])=>{
      this.color = data;
...
///dar de baja
    ...
    deleteColormp(id_color){
      swal.fire({
  title: 'Eliminar color de Materia Prima?',
...
this.rerender();
   ...
   const detalle = {
    id_bitacora: this.id_bitacora,
      acciones: 'Dio de baja un color de materia prima'
...
//registrar
 get f(){
   ...
   submit(){
this.loading=true;
    this.service.repetido(this.form.get('color').value).subscribe((data: Colormp)=>{
         this.cantidad = data;
...
this.form.reset();
   ...
   const detalle = {
    id_bitacora: this.id_bitacora,
      acciones: 'Actualizó un color de materia prima'
      // Agrega aquí otros campos si es necesario
    };
